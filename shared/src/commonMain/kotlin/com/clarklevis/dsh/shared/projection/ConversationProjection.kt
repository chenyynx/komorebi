...
class ConversationProjector(
    ...
    private fun fold(record: SessionEvent) {
        ...
        when {
            ...
            event.type == "user/message" && !event.text.isNullOrEmpty() -> mutableItems += ConversationItem(
                id = eventId(record),
            ...
            event.type == "assistant/chunk" && event.chunkType == "text-delta" && key !in finalizedKeys ->
                appendStream("stream-text-$key", "text-$key", ConversationItemKind.ASSISTANT, labels.streamingAssistant, event.text.orEmpty(), date)
            event.type == "assistant/chunk" && event.chunkType == "reasoning-delta" && key !in finalizedKeys ->
                appendStream("stream-reason-$key", "reason-$key", ConversationItemKind.REASONING, labels.streamingReasoning, event.text.orEmpty(), date)
            event.type == "assistant/chunk" && event.chunkType == "tool-call-delta" && key !in finalizedKeys -> {
                ...
                appendStream("stream-tool-$toolKey", "tool-$toolKey", kind, name, event.tool?.argumentsDelta.orEmpty(), date)
            }
            event.type == "assistant/message" -> {
                finalizedKeys += key
                removeStream("text-$key")
                removeStream("reason-$key")
                event.reasoning?.takeIf(String::isNotEmpty)?.let { reasoning ->
                    mutableItems += ConversationItem("${eventId(record)}-reason", ConversationItemKind.REASONING, "Think", reasoning, epochSeconds = date)
                }
                if (!event.text.isNullOrEmpty() || !event.images.isNullOrEmpty()) {
                    mutableItems += ConversationItem(eventId(record), ConversationItemKind.ASSISTANT, "DeepSeek", event.text.orEmpty(), event.images.orEmpty(), epochSeconds = date)
                }
            }
            event.type == "tool/call" -> {
                val name = event.name ?: "Tool Call"
                val kind = if (name.equals("run_code", ignoreCase = true)) ConversationItemKind.JSON_TOOL else ConversationItemKind.TOOL
                mutableItems += ConversationItem(eventId(record), kind, name, event.arguments?.jsonDisplayText().orEmpty(), epochSeconds = date)
            }
            event.type == "tool/result" -> mutableItems += ConversationItem(
                id = eventId(record),
    ...
    private fun appendStream(
        ...
        if (index != null) {
            ...
            mutableItems += ConversationItem(id, kind, title, delta, epochSeconds = date)
        }
    }

    private fun removeStream(key: String) {
